...
async def login_google(token: str, user_srv: UserSrv = Depends(UserSrv)):
    try:
        ...
        try:
            ...
            user = await user_srv.new_user(id_info["sub"], id_info["email"], id_info["name"], id_info["picture"])
...
async def protected(user_id: UUID = Depends(auth_required), user_srv: UserSrv = Depends(UserSrv)):
    user = await user_srv.get_user(user_id)
    return {f"message": f"Hello {user.name} from a protected endpoint!"}
